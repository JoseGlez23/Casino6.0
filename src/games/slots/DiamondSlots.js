...
// Hook de sonidos para Diamond Slots - Mismo que AnimalSlots
const useGameSounds = () => {
...
// Símbolos del juego Diamond Slots
const symbols = [
  {
    id: "diamond",
    name: "Diamante",
...
// Componente de carrete - Mismo estilo que AnimalSlots
...
export default function DiamondSlots({ navigation }) {
  ...
  const spinReels = async () => {
    ...
    setCooldown(true);
    setStoppedReels(0);
    subtractCoins(bet, `Apuesta en Diamond Slots`);
    setLastWin(0);
    setWinningLines([]);

    // SONIDOS INICIALES
    await playSound("click");
    await playSound("maquinita"); // Sonido de máquina al comenzar
    pulseAnimation();
    ...
    setTimeout(async () => {
      ...
      if (winResult.hasWin) {
        ...
        addTickets(winResult.tickets, `Ganancia en Diamond Slots`);

        const isJackpot = winResult.winningLines.some(
          (line) => line.symbol.id === "diamond" && line.line === 0
        );
      ...
      >
        ...
        <View style={styles.slotsMachine}>
          <View style={styles.machineHeader}>
            <Text style={styles.machineTitle}>DIAMOND FORTUNE</Text>
          </View>
        ...
        >
          <Animated.View style={{ transform: [{ scale: pulseAnim }] }}>
            <Ionicons
              name={spinning ? "sync" : cooldown ? "timer" : "diamond"}
              size={28}
              color="#FFF"
            />
        ...
        {cooldown && (
          <View style={styles.cooldownIndicator}>
            <Ionicons name="timer" size={16} color="#00BFFF" />
